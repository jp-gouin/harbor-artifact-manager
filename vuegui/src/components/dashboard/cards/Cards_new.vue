<template>
  <div  class="animate"> 
      <Cardbase v-bind:class="[ packageStatus=='outdated' ? 'border-orange-400 shadow-lg': packageStatus=='uptodate' ? 'border-green-400 shadow-lg':'border-gray-400' ]" :icon="icon" :title="name">
        <div slot="subheader" v-if="repo">
          <span class="uppercase text-xs text-gray-600">Repo : </span>
          <span>{{repo}}</span>
        </div>
        <div v-if="date" slot="rightheader" >
            ...
            <button v-else class="mr-1 m-auto flex rounded-full items-center uppercase  text-xs bg-gray-200 border border-gray-400">
          ...
          <div slot="content" class="flex flex-col flex-wrap">
            <slot name="content">
...
<script>
import Cardbase from './Cards_base.vue'
export default {
    name: 'Cards',
    components:{Cardbase},
    props: {
